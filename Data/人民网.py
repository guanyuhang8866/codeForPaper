...
def main():
    ...
    for k in range(0,1):
        print("城市：石家庄")
        with open("石家庄" + ".txt", "a", encoding="utf-8") as json_file:
            for i in range(1, 1001):
                print("page:",i)
                url = "http://search.people.com.cn/cnpeople/search.do?pageNum=" + str(i) + "&keyword=" +urllib.parse.quote('石家庄'.encode("gb2312")) + "&siteName=news&facetFlag=true&nodeType=belongsId&nodeId=0"
                while 1:
                    print("try!")
                    dirver.get(url)
